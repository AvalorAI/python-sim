...
def main():
    ...
    vehicle = connector.setup_mavlink_connection(n=0)
    ...
    try: 
        ...
        while True:
            ...
            # Update MAVLink
            connector.send_heartbeat(time_absolute_seconds, vehicle)
            connector.send_gps(time_absolute_microseconds, t__microseconds, quad, vehicle)
            connector.send_state_quaternion(time_absolute_microseconds, t__microseconds, quad, vehicle)
            connector.send_sensor(time_absolute_microseconds, t__microseconds, quad, vehicle)
            actuators = connector.receive_actuator_outputs(vehicle)
            
            print(f'actuators: {actuators}')
            # Update sensor state
            if actuators is not None:
                state = physics.update_state_rk4(physics.state, actuators, DT, GROUND_LEVEL)
